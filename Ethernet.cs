...
namespace BiometricServer
{
    public class CS
    {
        ...
        public void Init(SQLiteConnection connnection)
        {
            conn = connnection;
        }
        ...
        //----------------------------------------------------------------
        {
            ...
            int port = int.Parse(ReadSetting("SERVER_PORT"));
            ...
            {
                LogManager.DefaultLogger.Info("[TCPIP -> Server]  tcpListener.Start():\r\n" + ex.ToString());
            }
            ...
            while (!IsStopped)
            {
                try
                {
                    ...
                    LogManager.DefaultLogger.Info("[TCPIP -> Server] DB-Matik Client has connected");
                    ...
                    if (task.IsFaulted)
                    {
                        LogManager.DefaultLogger.Info("[TCPIP -> Server] if (task.IsFaulted) entered");
                        await task;
                    }
                }
                catch(Exception)
                {
                    //LogManager.DefaultLogger.Info("[TCPIP -> Server] Exception2:\r\n" + ex.ToString());
                }
            }
        }
        ...
        //----------------------------------------------------------------
        {
            ...
            {
                // log the error
                LogManager.DefaultLogger.Info("[TCPIP -> Server] Exception1:\r\n" + ex.ToString());
            ...
            return Task.Run(async () =>
            {
                using (var networkStream = tcpClient.GetStream())
                {
                    ...
                    while (true)
                    {
                        ...
                        if (byteCount > 2)
                        {
                            LogManager.DefaultLogger.Info("[TCPIP -> Server] Client wrote:\r\n" + request);
                            ...
                            if (request.Contains("</Evt>"))
                            {
                                ...
                                LogManager.DefaultLogger.Info("[TCPIP -> Server] ACK Response has been written");
                            }
                         }
                    }
        ...
        //----------------------------------------------------------------
        {
            ...
            while(!IsStopped)
            {
                ...
                if (RequestCompletet.Contains("</Evt>"))
                {
                    ...
                    LogManager.DefaultLogger.Info("[Worker] Client wrote:\r\n" + Request);
                }
                else
                {
                    continue;
