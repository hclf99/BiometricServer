...
namespace BiometricServer
{
    public class CS3
    {
        ...
        //----------------------------------------------------------------
        {
            ...
            if (tcpListener != null)
            {
                LogManager.DefaultLogger.Info("TCPIP -> Server3] stopping Listener 3");
                tcpListener.Stop();
            }
        }
        ...
        //----------------------------------------------------------------
        {
            ...
            int port = int.Parse(ReadSetting("SERVER_PORT3"));

            LogManager.DefaultLogger.Info("TCPIP -> Server3] starting Listener on Port: " + port.ToString());
            ...
            {
                LogManager.DefaultLogger.Error("TCPIP -> Server3]  Exception tcpListener.Start():\r\n" + ex.ToString());
            }
            ...
            while (!IsStopped)
            {
                try
                {
                    ...
                    LogManager.DefaultLogger.Info("TCPIP -> Server3] DB-Matik Client has connected");
                    ...
                    if (task.IsFaulted)
                    {
                        LogManager.DefaultLogger.Error("TCPIP -> Server3] if (task.IsFaulted) entered");
                        await task;
                    }
                }
                catch(Exception)
                {
                    //LogManager.DefaultLogger.Info("TCPIP -> Server3] Exception2:\r\n" + ex.ToString());
                }
            }
        }
        ...
        //----------------------------------------------------------------
        {
            ...
            {
                // log the error
                LogManager.DefaultLogger.Error("TCPIP -> Server3] Exception1:\r\n" + ex.ToString());
            ...
            return Task.Run(async () =>
            {
                using (var networkStream = tcpClient.GetStream())
                {
                    ...
                    while (true)
                    {
                        ...
                        if (byteCount > 2)
                        {
                            LogManager.DefaultLogger.Info("TCPIP -> Server3] Client wrote:\r\n" + request);
                            ...
                            if (request.Contains("</Evt>"))
                            {
                                ...
                                LogManager.DefaultLogger.Info("TCPIP -> Server3] ACK Response has been written");
                            }
                         }
                    }
        ...
        //----------------------------------------------------------------
        {
            ...
            while(!IsStopped)
            {
                ...
                if (RequestCompletet.Contains("</Evt>"))
                {
                    ...
                    LogManager.DefaultLogger.Info("[Worker3] Client wrote:\r\n" + Request);
                }
                else
                {
                    continue;
        ...
        //----------------------------------------------------------------
        {
            ...
            try
            {
                string result = appSettings[key] ?? "Not Found 3";
                return result;
            }
